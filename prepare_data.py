# python prepare_data.py args.json --pretrained_m2m
...
def main():
    parser = argparse.ArgumentParser(
        description='Add new languages to an existing m2m model.'
    )
    parser.add_argument(
        "--pretrained_m2m", action='store_true',
        help="Whether to use a pre-trained m2m or the custom m2m built " \
    ...
    # Check if any of the languages were not in m2m
    new_language_codes = []
    new_langs = {}
    ...
    for lang in new_langs.keys():
        if new_langs[lang]:
            new_language_codes.append(
                '__{}__'.format(lang.lower())
            )
            print("New language added:", lang)
    ...
    # If the flag `--pretrained_m2m` is used, then a m2m model is loaded
    # otherwise a transformer with customisable architecture (specified in
    # the arguments file)
    if sys_args.pretrained_m2m:
        # Map the vocabulary of the original m2m and the new one, i.e.
        ...
        # Check if the specified embedding length is the same as m2m's
        m2m_embedding_length = model.get_input_embeddings().weight.shape[1]
        if m2m_embedding_length != args.embedding_length:
            raise ValueError(
                'The m2m embedding length {} does not match {} length ' \
                'specific by args.embedding_length. To use a custom length, ' \
                'disable the --pretrained_m2m option to prepare a custom ' \
                'transformer model.'.format(
                    m2m_embedding_length,
